    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if payment.get('state') == 0:
                ...
                raise Exception(f'Error creating Heleket payment: status={response.status_code}, response={response.text}', )
        else:
...
def heleket_webhook(request):
    ...
    try:
        ...
        try:
            order = Order.object.select_for_update().get(id=order_id)
        except Order.DoesNotExist:
